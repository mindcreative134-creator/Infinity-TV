...
class _SeriesScreenState extends State<SeriesScreen> {
  ...
  final List<Filmy4uHDMedia> _series = [];
  bool _loading = true;
  bool _hasMore = true;
  int _page = 1;
  ...
  Future<void> _fetch() async {
    ...
    final results = await _service.getTVShows(page: _page);

    if (mounted) {
      setState(() {
        _series.addAll(results);
        _loading = false;
  ...
  Future<void> _refresh() async {
    _service.clearCache();
    setState(() { _series.clear(); _page = 1; _hasMore = true; });
    await _fetch();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFF0A0A0A),
        elevation: 0,
        title: const Text('TV Series', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 20)),
        actions: [
          IconButton(
        ...
        child: _series.isEmpty && _loading
            ...
            : _series.isEmpty
                ...
                : GridView.builder(
                    ...
                    itemCount: _series.length + (_hasMore ? 3 : 0),
                    itemBuilder: (ctx, i) {
                      if (i >= _series.length) return _shimmerCard();
                      return _SeriesCard(media: _series[i]);
                    },
                  ),
      ),
    );
  ...
  Widget _empty() => Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.tv_off_rounded, color: Colors.white.withValues(alpha: 0.1), size: 80),
          const SizedBox(height: 16),
          const Text('No series found', style: TextStyle(color: Colors.grey, fontSize: 16)),
...
class _SeriesCard extends StatelessWidget {
  ...
  const _SeriesCard({required this.media});
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  child: Row(children: [
                    ...
                    const Spacer(),
                    if (media.totalSeasons != null)
                      Text('S${media.totalSeasons}', style: const TextStyle(color: Colors.white70, fontSize: 9)),
                  ]),
                ),
              ),
            ]),
